...
DIRECTION=3
# Speed
SPEED_DOWNLOAD="100Gbit"
SPEED_UPLOAD="100Gbit"
# Delay in milliseconds (0 = no delay)
DELAY=700

function show_usage {
    echo
    echo "Bandwidth Control using TC, required arguments for 2 IP addresses at
    the same time"
    echo
    echo "Usage: $1 [-r|--remove] [-i|--incoming] [-o|--outgoing] [-d|--delay=]
    [--uspeed=] [--dspeed=] <IP> <IP>"
    echo
    echo "Arguments:"
    ...
    echo "  <IP> <IP>            : the ip addresses to limit the traffic for"
    echo
}
...
function tc_outgoing {
    ...
    echo "- filter on IP $ADDRESS $ADDRESS2"
    TCF="tc filter add"
    ...
    $TCF dev $INTERFACE protocol ip parent 1: prio 1 u32 match ip dst $ADDRESS2 flowid 1:1
}
...
function tc_incoming {
    ...
    $TCF dev $VIRTUAL protocol ip parent 2: prio 1 u32 match ip src $ADDRESS2 flowid 2:1
}
