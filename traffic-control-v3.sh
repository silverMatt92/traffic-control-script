...
function show_usage {
    ...
    echo "  <IP> <IP> <IP> ...            : the ip address/subnet list, to limit the traffic for"
    echo
}
...
function tc_outgoing () {
    ...
    for ip in "$@"
    do 
        echo "- filter on IP $ip"
    done
    TCF="tc filter add"
    if [[ $(tc filter show dev $INTERFACE | grep '^filter parent 1: protocol ip pref 1') ]]; then
        TCF="tc filter change"
    fi
    # Looping through cli arguments passed to the function and filtering
    for ip in "$@"
    do 
        $TCF dev $INTERFACE protocol ip parent 1: prio 1 u32 match ip dst $ip flowid 1:1
    done
}
...
function tc_incoming () {
    ...
    for ip in "$@"
    do
        $TCF dev $VIRTUAL protocol ip parent 2: prio 1 u32 match ip src $ip flowid 2:1
    done
}
